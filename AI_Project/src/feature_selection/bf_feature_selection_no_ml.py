#!/usr/bin/env python3
"""
Best-First Feature Selection (information-gain only, no ML evaluation)
...
- Best-First selection: priority queue by IG; select top-N features greedily.
- Save artifacts (features.csv, information_gain.csv, selection_history.csv,
  selected_features.json) and an IG bar-plot.
...
Usage examples:
  python src/feature_selection/bf_feature_selection_no_ml.py \
    ...
    --max-features 30 \
    --use-candidates src/preprocess/data/processed/candidates/candidates.csv
"""
...
def main():
    parser = argparse.ArgumentParser("Best-First IG feature selection (no-ML)")
    ...
    # Best-first selection by IG (no ML)
    selected, history = best_first_by_ig(ig, max_features=args.max_features)
    # save selection history
    hist_df = pd.DataFrame([{"iteration": it, "feature": feat, "information_gain": igv} for it,feat,igv in history])
    hist_path = os.path.join(args.output_dir, "selection_history.csv")
    hist_df.to_csv(hist_path, index=False)
    print(f"Wrote selection_history.csv -> {hist_path}")

    # save selected features
    sel_json = {"selected_features": selected, "parameters": {"max_features": args.max_features, "canonical_threshold": args.canonical_threshold}}
